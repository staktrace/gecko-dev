 ...
 * synchronous API to deliver incoming input events to APZ and modify them
 * in-place to unapply the APZ async transform. If there is a GPU process,
 * then this class does sync IPC calls over to the GPU process in order to
 * accomplish this. Otherwise, APZCTreeManager overrides and implements
