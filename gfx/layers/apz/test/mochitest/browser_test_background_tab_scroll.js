add_task(async function test_main() {
  ...
  let url = "http://mochi.test:8888/browser/gfx/layers/apz/test/mochitest/helper_background_tab_scroll.html#scrolltarget";
  ...
  let scrollPos = await ContentTask.spawn(
    browser,
    null,
    function() {
  ...
  ok(scrollPos < 5000, `Expected background tab to have scrolled up, is at ${scrollPos}`);
