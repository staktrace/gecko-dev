...
nsDisplayTransform::GetTransformForRendering(LayoutDevicePoint* aOutOrigin)
{
  if (!mFrame->HasPerspective() || mTransformGetter || mIsTransformSeparator) {
    if (!mFrame->HasPerspective() && aOutOrigin) {
                                            ...
                                            nsDisplayListBuilder* aDisplayListBuilder)
{
  ...
  // For reference frames we want to make sure we don't pollute the transform
  // property in the WR stacking context by including the position of this
  // frame (relative to the parent reference frame). We need to keep those
  // separate; the position of this frame goes into the stacking context bounds
  // while the transform goes into the transform.
  LayoutDevicePoint position;
  Matrix4x4 newTransformMatrix = GetTransformForRendering(isReferenceFrame ? &position : nullptr);
  ...
  AddAnimationsForProperty(Frame(), aDisplayListBuilder,
                           ...
                           animationInfo, false, isReferenceFrame);
  animationInfo.StartPendingAnimations(aManager->GetAnimationReadyTime());
